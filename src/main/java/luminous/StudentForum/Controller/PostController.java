...
public class PostController {
    ...
    public String CategoryCreate(@Valid Post post, BindingResult result, HttpSession session, HttpServletRequest req,
        ...
        if (req.getParameter("category").equals("--Select a Category--")) {
            ...
            // result.addError(new FieldError("category", "category", "Username Already
            // exists!!"));
        }
        if (result.hasErrors()) {
    ...
    public ModelAndView ViewSinglePost(@PathVariable("id") int id, @PathVariable("cat") String cat,
    ...
    public ModelAndView ViewAllPost(@PathVariable("page") int page, ModelAndView modelAndView, HttpSession session,
            Model model) {
    ...
    public ModelAndView ViewCtegoryPost(@PathVariable("page") int page, @PathVariable("cat") String cat,
    ...
    public ModelAndView ViewBySearchPost(@PathVariable("page") int page, @PathVariable("shrc") String shrc,
            ModelAndView modelAndView) {
    ...
    public String DeletePost(@PathVariable("id") int id) {
        log.info("I am on delete post ");
        log.info(postRepo.findById(id));
        postRepo.delete(postRepo.findById(id));
        return "redirect:/";
    }
    @GetMapping("posts/edit/{id}")
    public ModelAndView EditPost(@PathVariable("id") int id, ModelAndView modelAndView) {
        modelAndView.addObject("category", catRepo.findAll());
        modelAndView.addObject("post", new Post());
        modelAndView.addObject("oldPost", postRepo.getAllPostDetailsByID(id));
        modelAndView.setViewName("post-edit");
        return modelAndView;
    }
    @PostMapping("posts/edit/{id}")
    public String UpdatePost(@PathVariable("id") int id, HttpSession session, HttpServletRequest req) {
        log.info(req.getParameter("category"));
            Post post = postRepo.findById(id);
            post.setCreatedAt(new Date(System.currentTimeMillis()));
            post.setPbody(req.getParameter("pbody"));
